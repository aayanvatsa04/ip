...
When creating or proposing a commit message for the reviewed changes, follow the
repository's commit conventions in `AGENTS.md`: an imperative subject plus a body
with enough detail to explain the rationale for the change. Do not commit or push
...
3. Write to `_temp/visual-diff.html` unless the user supplies an output path. `_temp/` is already gitignored in this repository, so generated pages stay out of version control.
4. Run the bundled generator from the repository root:
   ...
   Useful flags: `--no-unchanged` omits the collapsed panels for files that did not change, and `--open` opens the page in a browser when done.
...
`scripts/generate-split-view-diff.py` is the bundled generator. It needs only Python's standard library — no `pip install` step. Syntax highlighting is loaded in the browser from a CDN; without a network the page still works, just without colored tokens. Keep the page self-contained apart from those optional syntax-highlighting resources.
